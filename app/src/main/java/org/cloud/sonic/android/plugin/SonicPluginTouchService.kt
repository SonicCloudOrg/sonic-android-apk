...
class SonicPluginTouchService(var width: Int = 0, var height: Int = 0, var handler: Handler?) :
  ...
  companion object {
    ...
    fun getScreenSize(): Point? {
      val displayManager: Any = getInstance("android.hardware.display.DisplayManagerGlobal")
      try {
        val displayInfo =
  ...
  private fun manageClientConnection() {
    while (!hasStop) {
      LogUtils.i(String.format("Listening on %s", SOCKET))
  ...
  override fun run() {
    serverSocket = try {
      LogUtils.i(String.format("creating socket %s", SOCKET))
