...
import org.cloud.sonic.android.recorder.RecordConfig;
...
public class Mp3EncodeThread extends Thread {
    ...
    public Mp3EncodeThread(File file, int bufferSize) {
        this.file = file;
        mp3Buffer = new byte[(int) (7200 + (bufferSize * 2 * 1.25 * 2))];
        RecordConfig currentConfig = new RecordConfig();
        ...
        Mp3Encoder.init(sampleRate/2, currentConfig.getChannelCount(), sampleRate, currentConfig.getRealEncoding(),0);
    }
